...
class YesVsNoByPriceRtAnalysis(Analysis):
    """Analyze YES/NO preference by price, comparing takers vs makers."""
    ...
    def run(self) -> AnalysisOutput:
        ...
        df = con.execute(
            ...
            WITH rt_markets AS (
                ...
                WHERE event_ticker LIKE 'RT%'
            ),
            taker_yes AS (
                SELECT t.yes_price AS price, SUM(t.count) AS contracts
                FROM '{self.trades_dir}/*.parquet' t
                INNER JOIN rt_markets m ON t.ticker = m.ticker
                WHERE t.taker_side = 'yes'
                GROUP BY t.yes_price
            ),
            taker_no AS (
                SELECT t.no_price AS price, SUM(t.count) AS contracts
                FROM '{self.trades_dir}/*.parquet' t
                INNER JOIN rt_markets m ON t.ticker = m.ticker
