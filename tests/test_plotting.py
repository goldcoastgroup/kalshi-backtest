...
def test_plot_no_market_prices():
    ...
    from src.backtesting.plotting import plot
    fig = plot(_make_result(include_prices=False), open_browser=False,
               filename="/tmp/test_plot_no_prices")
    assert fig is not None
...
def test_plot_no_fills():
    ...
    from src.backtesting.plotting import plot
    fig = plot(_make_result(n_fills=0, include_prices=False, include_pnls=False),
               open_browser=False, filename="/tmp/test_plot_no_fills")
    assert fig is not None
...
def test_plot_minimal():
    """Very few snapshots."""
    from src.backtesting.plotting import plot
    fig = plot(_make_result(n_snaps=3, n_fills=1, n_markets=1),
               open_browser=False, filename="/tmp/test_plot_minimal")
    assert fig is not None
...
def test_plot_absolute_equity():
    ...
    from src.backtesting.plotting import plot
    fig = plot(_make_result(), open_browser=False, relative_equity=False,
               filename="/tmp/test_plot_absolute")
    assert fig is not None
...
def test_plot_no_pnls():
    ...
    from src.backtesting.plotting import plot
    fig = plot(_make_result(include_pnls=False), open_browser=False,
               filename="/tmp/test_plot_no_pnls")
    assert fig is not None
